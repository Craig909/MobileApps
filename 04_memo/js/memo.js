...
function saveLocalStorage(){
    ...
    function(e){
        ...
        const value=document.getElementById("testMemo").value;
        ...
        if(key==""||value==""){
            ...
            if(w_confirm === true){
                ...
                document.getElementById("testMemo").value="";
            }
         }
    },false
    );
...
function allClearLocalStorage(){
    ...
    function(e){
        e.preventDefault();
        let w_confirm=window.confirm("localStorageのデータをすべて削除します。よろしいですか？");
...
function selectCheckBox(mode){
    ...
    document.getElementById("testKey").value=w_textKey;
        document.getElementById("testMemo").value=w_textMemo;
    if(mode=="select"){
        if(w_cnt ===1){
            return w_cnt;
        }else{
...
//sortlist:引数１...最初からソートしておく列を指定、引数２...0...
$("#table1").tablesorter({
    sortList:[[1,0]]
});
